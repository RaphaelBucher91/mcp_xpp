...
namespace D365MetadataService.Handlers
{
    ...
    /// Uses subAction parameter to distinguish between operations:
    ///   - "get": Get a single label by reference
    ///   - "batch": Get multiple labels efficiently
    /// </summary>
    public class LabelHandler : BaseRequestHandler
    {
        ...
        protected override Task<ServiceResponse> HandleRequestAsync(ServiceRequest request)
        {
            ...
            {
                Logger.Error(ex, "[Label] Error handling label request");
        ...
        private ServiceResponse HandleBatchLabels(ServiceRequest request)
        {
            ...
            {
                Logger.Error(ex, "[Label] Error parsing labelIds parameter");
