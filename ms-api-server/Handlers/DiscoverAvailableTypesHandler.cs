...
namespace D365MetadataService.Handlers
{
    ...
    public class DiscoverAvailableTypesHandler : BaseRequestHandler
    {
        ...
        protected override Task<ServiceResponse> HandleRequestAsync(ServiceRequest request)
        {
            try
            {
                Logger.Information("[DiscoverTypes] Discovering available D365 types");
                ...
                Logger.Information("[DiscoverTypes] Found {TypeCount} available D365 types", types.Count);

                return Task.FromResult(ServiceResponse.CreateSuccess(types));
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "[DiscoverTypes] Error discovering available D365 types");
        ...
        private List<string> GetAvailableObjectTypes()
        {
            ...
            try
            {
                ...
                Logger.Information($"[DiscoverTypes] Discovered {objectTypes.Count} object types from metadata assembly");
                
                // Log first few for debugging
                if (objectTypes.Count > 0)
                {
                    Logger.Information($"[DiscoverTypes] Sample object types: {string.Join(", ", objectTypes.Take(10))}");
                }
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "[DiscoverTypes] Error discovering object types from metadata assembly");
            ...
            if (objectTypes.Count == 0)
            {
                Logger.Warning("[DiscoverTypes] No object types discovered, returning empty list");
                return new List<string>();
            }
